...
class TronApi{
  ...
  static function mainNet($token = null){ 
    return new self('https://api.trongrid.io',$token);
  }
  
  static function testNet($token = null){ 
    return new self('https://api.shasta.trongrid.io',$token);
  }
  
  static function myNet($url,$token = null){ 
    if(substr($url,0,4) != "http"){
        throw new \Exception('new Tron() 请传入自定义的节点URL地址');
    }
    return new self($url,$token);
  }
  
  function __construct($fullNodeUrl,$token){  
        $solidityNodeUrl = $fullNodeUrl;
        $eventNodeUrl = $fullNodeUrl; 
        
        $this->fullNode = new NodeClient($fullNodeUrl,$token);
        $this->solidityNode = new NodeClient($solidityNodeUrl,$token);
        $this->eventNode = new NodeClient($eventNodeUrl,$token);
  ...
  */
  ...
  //get|post?
  function getNowBlock($confirmed=false){
    if($confirmed){
  ...
  function getTransactionById($txid,$confirmed=false){
    $payload = [
      'value' => $txid
    ];
    if($confirmed){
  ...
  function getTransactionInfoById($txid,$confirmed=false){
    $payload = [
      'value' => $txid
    ];
    if($confirmed){
  ...
  function getReward($address,$confirmed=false){
    $payload = [
      'address' => Address::decode($address)
    ];
    if($confirmed){
